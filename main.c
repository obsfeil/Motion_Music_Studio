/**
 * @file main.c
 * @brief MSPM0G3507 Synthesizer with LCD Display - FIXED
 * @version 1.3.2-fixed
 * @date 2025-12-19
 * 
 * CRITICAL FIXES:
 * - Removed floating point printf (causes Hard Fault on M0+ without proper config)
 * - Using manual float conversion as per technical report section 6.4
 * - Enabled LCD_Init() 
 * - Added missing ADC_ACCEL interrupt handler
...
//=============================================================================
// SAFE FLOAT TO STRING (No printf %f - prevents Hard Fault)
//=============================================================================

/**
 * @brief Converts float to string without using printf %f
 * @note As per technical report section 6.4, this avoids Hard Faults
 */
void float_to_string(float value, char *buffer, uint8_t buffer_size, uint8_t decimals) {
    int integer_part = (int)value;
    int decimal_part = (int)((value - integer_part) * 100);
    if (decimal_part < 0) decimal_part = -decimal_part;
    
    snprintf(buffer, buffer_size, "%d.%02d", integer_part, decimal_part);
}
...
//=============================================================================
// DISPLAY UPDATE (FIXED - No float printf)
//=============================================================================
...
static void Update_Display(void) {
    ...
    // CRITICAL FIX: Use manual float conversion instead of %.1f
    float_to_string(g_synthState.frequency, freq_str, sizeof(freq_str), 1);
    snprintf(str, sizeof(str), "F: %s Hz", freq_str);
...
// CRITICAL FIX: Added missing interrupt handler for ADC_ACCEL
...
// Forbedret HardFault handler med bedre diagnostikk
void HardFault_Handler(void) {
    ...
    LCD_DrawString(10, 80, "Check linker cfg", COLOR_WHITE);
...
int main(void) {
    // Step 1: Initialize hardware configuration
    SYSCFG_DL_init();

    // Step 2: CRITICAL - Explicit state initialization (per technical report)
    ...
    // Step 3: CRITICAL FIX - Initialize LCD (was commented out!)
    // Per technical report section 5, proper initialization sequence is essential
    LCD_Init();
    LCD_Clear(COLOR_BLACK);
    ...
    // Step 6: Enable interrupts (proper order per report)
    ...
    // Step 7: Initialize audio synthesis
    Update_Phase_Increment();
    
    // Step 8: Initial display
    Update_Display();

    // Main loop
    while (1) {
        check_stack_canary();  // Monitor stack health
        Process_Input();

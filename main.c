...
#define PWM_CENTER 8192
#define PWM_MAX    16383
...
#define UART_DECIMATION_FACTOR 2 // Send every 8th sample
//=============================================================================
// MAIN
//=============================================================================
int main(void) {
...
void TIMG7_IRQHandler(void) {
  ...
  if (gSynthState.audio_playing)
    ...
    DL_TimerG_setCaptureCompareValue(PWM_AUDIO_INST, 8192, DL_TIMER_CC_0_INDEX);
}
...
static void Generate_Audio_Sample(void) {
  ...
  if (gSynthState.volume == 0 || amplitude == 0) {
    // ✅ Output silence (center position = 8192 for 14-bit PWM)
    DL_TimerG_setCaptureCompareValue(PWM_AUDIO_INST, PWM_CENTER, DL_TIMER_CC_0_INDEX);
  ...
  // ✅ OPTIMIZED: 14-bit PWM for piezo buzzer (better frequency resolution)
  // Using defines from top of file: PWM_CENTER = 8192, PWM_MAX = 16383
  uint16_t pwm_val = Audio_SampleToPWM(sample, PWM_CENTER, PWM_MAX);
  DL_TimerG_setCaptureCompareValue(PWM_AUDIO_INST, pwm_val, DL_TIMER_CC_0_INDEX);
  
  // Send to PC via UART (keep for monitoring)
...
static int16_t Generate_Chord_Sample(volatile uint32_t *phases,
  ...
  for (uint8_t v = 0; v < num_voices; v++) {
    ...
    int16_t sample =
        Audio_GenerateWaveform(index, inst->waveform); // Library API

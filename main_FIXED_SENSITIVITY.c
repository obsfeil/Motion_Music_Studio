...
static const Preset_t PRESETS[3] = {{.name = "CLASSIC",
...
int main(void) {
  ...
  LCD_PrintString(10, 10, "FIXED v12.0", LCD_COLOR_GREEN, LCD_COLOR_BLACK,
                  FONT_LARGE);
  LCD_PrintString(5, 40, "Pitch: 10x", LCD_COLOR_CYAN, LCD_COLOR_BLACK,
                  FONT_MEDIUM);
  LCD_PrintString(5, 60, "  mindre", LCD_COLOR_CYAN, LCD_COLOR_BLACK,
                  FONT_MEDIUM);
  LCD_PrintString(5, 80, "  sensitiv!", LCD_COLOR_CYAN, LCD_COLOR_BLACK,
                  FONT_MEDIUM);
  LCD_PrintString(5, 100, "S1 fikset!", LCD_COLOR_YELLOW, LCD_COLOR_BLACK,
                  FONT_MEDIUM);
  ...
  while (1) {
    // Check if new DMA data is ready
    if (gADC0_DMA_Ready) {
      gADC0_DMA_Ready = false;

      // Process batch of samples
      Process_ADC0_Batch(gADC0_ProcessBuffer, ADC0_BUFFER_SIZE);
    }
...
void TIMG7_IRQHandler(void) {
  if (DL_Timer_getPendingInterrupt(TIMER_SAMPLE_INST) == DL_TIMER_IIDX_ZERO) {
    ...
    if (gSynthState.audio_playing) {
      ...
      DL_TimerG_setCaptureCompareValue(PWM_AUDIO_INST, 2048,
                                       DL_TIMER_CC_0_INDEX);
    }
  }
}

// Replace ADC0_IRQHandler with DMA handler

void DMA_IRQHandler(void) {
  uint32_t status = DL_DMA_getPendingInterrupt(DMA);

  if (status == DL_DMA_EVENT_IIDX_DMACH0) {
    // DMA Channel 0 transfer complete
    // Buffer is full, copy to processing buffer

    // Disable interrupts briefly to avoid race condition
    uint32_t primask = __get_PRIMASK();
    __disable_irq();

    // Copy DMA buffer to processing buffer
    memcpy(gADC0_ProcessBuffer, (const void *)gADC0_DMA_Buffer,
           sizeof(gADC0_ProcessBuffer));

    // Set flag for main loop
    gADC0_DMA_Ready = true;

    __set_PRIMASK(primask);

    // Reading IIDX automatically clears interrupt
  }
  // In DMA_IRQHandler, add:
  else if (status == DL_DMA_EVENT_IIDX_DMACH1) {
    // Process accelerometer data
    gSynthState.accel_x = gADC1_DMA_Buffer[0]; // Latest X
    gSynthState.accel_y = gADC1_DMA_Buffer[1]; // Latest Y
    gSynthState.accel_z = gADC1_DMA_Buffer[2]; // Latest Z
  }
}
...
void GPIOA_IRQHandler(void) {
  uint32_t pending = DL_GPIO_getEnabledInterruptStatus(
      GPIO_BUTTONS_PORT, GPIO_BUTTONS_S1_PIN | GPIO_BUTTONS_S2_PIN
  ...
  uint32_t current_time =
      gSynthState.timer_count; // Use timer count as timestamp

  // Check S1 button
  if (pending & GPIO_BUTTONS_S1_PIN) {
    if ((current_time - s1_last_press_time) >
        (DEBOUNCE_TIME_MS * 8)) { // 8kHz timer = 8 ticks per ms
...
static void Process_ADC0_Batch(const uint16_t *buffer, uint32_t length) {
  // ADC0 samples alternate: MEM0 (joy_y), MEM1 (joy_x)
  // For 64 samples: 32 joy_y + 32 joy_x readings

  // Calculate averages (simple filtering)
  uint32_t sum_y = 0;
  uint32_t sum_x = 0;
  uint32_t count = 0;

  for (uint32_t i = 0; i < length; i += 2) {
    sum_y += buffer[i];     // MEM0 = joy_y
    sum_x += buffer[i + 1]; // MEM1 = joy_x
    count++;
  }

  // Update global state with averaged values
  gSynthState.joy_y = sum_y / count;
  gSynthState.joy_x = sum_x / count;

  // This averaging also provides noise filtering! ✅
}
...
static void Process_Pitch_Bend(void) {
  ...
  int8_t semitones =
      (int8_t)((deviation * PITCH_BEND_MAX_SEMITONES) / PITCH_BEND_SENSITIVITY);

  if (semitones > PITCH_BEND_MAX_SEMITONES)
    semitones = PITCH_BEND_MAX_SEMITONES;
  if (semitones < -PITCH_BEND_MAX_SEMITONES)
    semitones = -PITCH_BEND_MAX_SEMITONES;
...
static void Display_Update(void) {
  ...
  LCD_PrintString(90, 18, "D:", LCD_COLOR_DARKGRAY, LCD_COLOR_BLACK,
                  FONT_SMALL);
  LCD_PrintNumber(102, 18, deviation, LCD_COLOR_DARKGRAY, LCD_COLOR_BLACK,
                  FONT_SMALL);
